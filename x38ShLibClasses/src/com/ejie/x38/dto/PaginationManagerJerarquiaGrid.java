...
public class PaginationManagerJerarquiaGrid implements java.io.Serializable{
	...
	public static StringBuilder getQuery(
		...
		query.append("\n\t").append(", sys_connect_by_path(").append(columna).append(", '").append(pagination.getJerarquia().getToken()).append("') as TREENODES ");
		return PaginationManagerJerarquia.getQuery(pagination, query, mapaWhere, columna, columnaPadre, columnaParentNodes, tabla, aliasTabla, joins, businessFilters, businessParams);
	}
	 ...
	 * SELECTED
	 */
	public static StringBuilder getQuerySelectedGrid(
			Pagination pagination,
			StringBuilder query,  Map<String, ?> mapaWhere,
			Object bean, 
		...
		return getQuerySelectedGrid(pagination, query, mapaWhere, bean, columna, columnaPadre, tabla, aliasTabla, new StringBuilder(""), null, null, new ArrayList<String>());
	}
	public static StringBuilder getQuerySelectedGrid(
			Pagination pagination,
			StringBuilder query,  Map<String, ?> mapaWhere,
		...
		return getQuerySelectedGrid(pagination, query, mapaWhere, bean, columna, columnaPadre, tabla, aliasTabla, joins, null, null, new ArrayList<String>());
	}
	public static StringBuilder getQuerySelectedGrid(
			Pagination pagination,
			StringBuilder query,  Map<String, ?> mapaWhere,
		...
		try {
	        ...
	        for (PropertyDescriptor pd : props) {  
	            ...
	            if (pd.getReadMethod().invoke(bean)!=null && !name.equals(pagination.getSidx())){
		...
		queryParams.add(pagination.getRows());
		queryParams.add(pagination.getRows());
		queryParams.add(pagination.getRows().toString()); //case requiere literal
		queryParams.add(pagination.getRows());
		query.append("\n\t").append("from ( ");
		...
		query.append("\n\t\t").append("order siblings by ").append(pagination.getSidx()).append(" ").append(pagination.getSord());
		query.append("\n\t").append(") ").append(aliasTabla.get(0)).append(", (");
		
		//Subqueries
			//PADRES
				query.append("\n\t\t").append("-- PADRES");
				query = PaginationManagerJerarquia.querySubquery(pagination, query, mapaWhere, queryParams, columna, tabla, aliasTabla, joins, businessFilters, businessParams);
		...
		//Nodos contraídos
		query = PaginationManagerJerarquia.filterUnexpanded(pagination, query, queryParams, columnaPadre);
		
		query.append("\n\t").append("order siblings by ").append(pagination.getSidx()).append(" ").append(pagination.getSord());
		query.append("\n").append(") ");

		//Filtrar seleccionados
		if (pagination.getJerarquia().getParentId()!=null){
			query.append("\n").append("-- Registros SELECCIONADOS");
			query.append("\n").append("where (1,pk) in (");
			StringBuilder selectedParams = new StringBuilder("");
			String[] selected = pagination.getJerarquia().getParentId().split(",");
			...
			for (int i = 0; i < selected_length; i++) {
				...
				elem = elem.substring(elem.lastIndexOf(token)+token.length());

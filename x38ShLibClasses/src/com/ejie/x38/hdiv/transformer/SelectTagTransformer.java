...
public class SelectTagTransformer implements ClassTransformer {
	...
	private void tryTransform() throws Exception {
		...
		modifyedMethod.setBody("{" + "    $1.startTag(\"select\");" + "	writeDefaultAttributes($1);"
				+ "	if (isMultiple()) {" + "		$1.writeAttribute(\"multiple\", \"multiple\");" + "	}"
				+ "	$1.writeOptionalAttributeValue(\"size\", getDisplayString(evaluate(\"size\", getSize())));"
				+ "	Object items = getItems();" + "	if (items != null) {" + "		if (items != EMPTY) {"
				+ "			Object itemsObject = evaluate(\"items\", items);" + "			if (itemsObject != null) {"
				...
				+ "				optionWriter.writeOptions($1);" + "			}" + "		}" + "		$1.endTag(true);"
				+ "		writeHiddenTagIfNecessary($1);" + "		return SKIP_BODY;" + "	}else {"
				+ "		processFieldValue($0.getName(), null, \"option\");" + "		$1.forceBlock();"
				+ "		this.tagWriter = $1;"
